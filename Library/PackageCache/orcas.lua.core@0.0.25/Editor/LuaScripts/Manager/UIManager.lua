...
UIManager.Init = function()
    ...
    -- 初始化层级
    for k, v in pairs(private.LayerNode) do
        local layerName = k;
        local nodeObj = v;
        
        if IsNil(nodeObj) == false then
            local depthControl = nodeObj.gameObject:GetComponent(typeof(UIDepthControl))
            if depthControl == nil then
                depthControl = nodeObj.gameObject:AddComponent(typeof(UIDepthControl))
            end
            local baseOrder = UISortingOrderEnum[layerName]
            depthControl:SetDepth(baseOrder)     
        end
    end
...
UIManager.GetInComponent = function(uiName, parentNode)
    ...
    if component.OnAwakeWithGameObject then
        component.OnAwakeWithGameObject(parentNode)
    end
    return component
end
...
private.SetUIDepth = function(ui)
    ...
    local depthControl = mTrans.gameObject:GetComponent(typeof(UIDepthControl))
    if depthControl == nil then
        ui.DepthControl = mTrans.gameObject:AddComponent(typeof(UIDepthControl))
    end
    local baseOrder = UISortingOrderEnum[ui.INIT_DATA.Depth]
    local o = ui.OrderOffset == nil and 1 or ui.OrderOffset
    ui.DepthControl:SetDepth(baseOrder + o)
end
